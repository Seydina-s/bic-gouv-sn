...
| 26/09/2026 | ADM-02b ✅ API de connexion `/admin/v1/auth` : mot de passe puis code à 6 chiffres (activation par QR code à la première connexion), secrets du second code chiffrés (AES-256-GCM, clé `ADMIN_SECRET_KEY` hors du code ; sans clé, routes absentes), réponse identique pour une adresse inconnue (même temps de calcul), verrouillage 15 min après 5 échecs, 10 essais/min par adresse, étape « code » valable 5 min, code utilisable une seule fois, session 30 min d’inactivité / 8 h max, jamais en cache, journal d’audit chaîné ; création du premier administrateur en ligne de commande (`pnpm --filter @bgs/api admin:create`, mot de passe masqué, messages en français) | Phase 4 |
| 26/09/2026 | ART-02 ✅ Bouton « Écouter » en verre dans le coin inférieur droit de la photo de l’article (à droite de la rubrique sans photo) ; lecture en français par la voix du téléphone (titre puis texte, découpé en phrases), « Arrêter » pendant la lecture, arrêt automatique en quittant l’article ; absent des articles wolof | Retour utilisateur |
| 26/09/2026 | NAV-02 🔴 Bouton « Revenir en haut » sur tous les écrans qui défilent (rubrique, article, recherche, favoris, démarches, fiche démarche) | Retour utilisateur |
| 26/09/2026 | SET-03 🔴 Réglages en fenêtre superposée : l’écran d’origine reste en place dessous (visible, flouté) et intact après fermeture, sans effet de « balayage » à l’ouverture | Retour utilisateur |
| 26/09/2026 | SET-01 ✅ Réglages en feuille de verre par-dessus la page (floutée sur iPhone et web ; verre presque opaque sur Android pour la fluidité), fermeture par un toucher à l’extérieur, la croix ou le geste retour ; thème (Auto / Clair / Sombre) et langue (Auto / Français / Wolof) en choix côte à côte avec icônes, façon barre d’onglets ; texte sur le wolof retiré ; couche de verre partagée avec la barre d’onglets (`GlassBackdrop`) | Retour utilisateur |
| 26/09/2026 | SET-02 ✅ « À propos » réécrit : actualité officielle FR/WO dès sa publication, démarches expliquées depuis e-senegal.sn, sources toujours citées, favoris lisibles hors ligne (uniquement ce que l’application fait déjà) | Retour utilisateur |
